# Contact & Support

**Zarq Messenger - We're Here to Help!** 💬

---

## 📧 Get in Touch

We value your feedback and are here to assist you with any questions, issues, or concerns.

### Primary Contact

**Email:** zarqmessenger@gmail.com

**Response Time:** We aim to respond within 7 business days.

**Developer:** Independent Developer based in India

---

## 🆘 How We Can Help

### Technical Support

Contact us if you experience:
- App crashes or bugs
- Login or registration issues
- Message delivery problems
- Connection errors
- Video/voice call issues
- File upload/download problems
- Notification issues

### Account Issues

We can help with:
- Account access problems
- Username changes
- Profile issues
- Password/authentication problems
- Account deletion requests
- Data export requests

### Privacy & Security

Contact us about:
- Privacy concerns
- Security vulnerabilities (please report responsibly)
- Data protection questions
- Encryption questions
- Suspicious activity

### Feature Requests

We'd love to hear:
- Ideas for new features
- Suggestions for improvements
- User experience feedback
- Design suggestions

### Legal & Compliance

For legal matters:
- Copyright concerns
- Terms of Service questions
- Privacy Policy inquiries
- Legal requests (must include proper documentation)

### General Inquiries

Feel free to ask about:
- How Zarq Messenger works
- End-to-end encryption
- Server locations
- Future plans
- Partnership opportunities

---

## 📝 When Contacting Us

### Please Include:

To help us assist you faster, please provide:

**For Technical Issues:**
- Your username
- Device model (e.g., Samsung Galaxy S23)
- Android version (e.g., Android 13)
- App version (found in Settings → About)
- Description of the problem
- Steps to reproduce the issue
- Screenshots (if applicable)

**For Account Issues:**
- Your username
- Phone number (last 4 digits only for security)
- Description of the issue

**For Security Reports:**
- Detailed description of the vulnerability
- Steps to reproduce
- Potential impact
- Your contact information

**For Feature Requests:**
- Description of the feature
- Why it would be useful
- How you envision it working

---

## ⚠️ Important Notes

### What We Can Do:
- ✅ Help with technical issues
- ✅ Answer questions about the app
- ✅ Assist with account problems
- ✅ Consider feature requests
- ✅ Address privacy concerns

### What We Cannot Do:
- ❌ Read your encrypted messages (due to E2EE)
- ❌ Recover deleted messages
- ❌ Access other users' accounts
- ❌ Bypass security measures
- ❌ Provide instant 24/7 support

### Privacy Note:
When contacting us:
- Do NOT include your full phone number in emails
- Do NOT share your password
- Do NOT share sensitive personal information
- We will NEVER ask for your password

---

## 🐛 Found a Bug?

### How to Report Bugs

**Minor Bugs:**
- Email us with details
- Include screenshots if possible
- Describe steps to reproduce

**Critical Bugs (App Crashes, Security Issues):**
- Email immediately: zarqmessenger@gmail.com
- Use subject line: "URGENT: [Brief Description]"
- Include all relevant details

**Security Vulnerabilities:**
- Email: zarqmessenger@gmail.com
- Subject: "SECURITY: Vulnerability Report"
- Please report responsibly (don't publicly disclose until fixed)
- We appreciate your help keeping Zarq secure!

---

## 💡 Self-Help Resources

### Before Contacting Support:

**Try these solutions first:**

**1. App Not Working?**
- Restart the app
- Check your internet connection
- Update to the latest version
- Clear app cache (Settings → Apps → Zarq → Clear Cache)
- Restart your phone

**2. Messages Not Sending?**
- Check internet connection
- Verify recipient's username
- Try sending again
- Check if recipient blocked you

**3. Not Receiving Notifications?**
- Check notification permissions (Settings → Apps → Zarq → Notifications)
- Ensure battery optimization is off for Zarq
- Check "Do Not Disturb" mode
- Verify notification settings in-app

**4. Can't Login?**
- Verify phone number is correct
- Check internet connection
- Wait for SMS verification code
- Try requesting code again
- Ensure you're entering the correct code

**5. Call Quality Issues?**
- Check internet speed (need at least 1 Mbps)
- Close background apps
- Switch from WiFi to mobile data (or vice versa)
- Ensure camera/microphone permissions are granted

---

## 🌟 Feedback & Suggestions

We're constantly improving Zarq Messenger!

### We'd Love to Hear:

**What's Working Well:**
- Features you love
- What makes Zarq useful for you
- Why you chose Zarq

**What Could Be Better:**
- Features you'd like to see
- User experience improvements
- Design suggestions
- Performance issues

**How to Share Feedback:**
- Email: zarqmessenger@gmail.com
- Subject: "Feedback: [Your Topic]"

---

## 🚨 Report Abuse or Violations

### If You Encounter:

- Harassment or bullying
- Spam messages
- Illegal content
- Impersonation
- Security threats

**Please Report:**
- Email: zarqmessenger@gmail.com
- Subject: "Report: [Issue Type]"
- Include: Username of violator, screenshots, description

**We take violations seriously and will:**
- Investigate all reports promptly
- Take appropriate action (warnings, account suspension/termination)
- Protect reporter identity

---

## 📱 Future Contact Channels

We're planning to add more support channels:

**Coming Soon:**
- In-app support chat
- FAQ section
- Video tutorials
- Community forum
- Social media support

**Stay tuned for updates!**

---

## 🌍 Social Media

**Coming Soon:**
- Twitter/X: @ZarqMessenger (planned)
- Instagram: @zarqmessenger (planned)
- GitHub: github.com/zarq-messenger (planned)

*We'll update this page once social media channels are active.*

---

## ⏰ Support Hours

**Email Support:**
- Available: 7 days a week
- Response time: Within 7 business days
- Priority given to critical issues

**Note:** As Zarq Messenger is developed by an independent developer, response times may vary. We appreciate your patience!

---

## 🔐 Security Contact

**For security vulnerabilities or critical security issues:**

**Email:** zarqmessenger@gmail.com
**Subject:** "SECURITY: [Brief Description]"

**Please include:**
- Detailed vulnerability description
- Steps to reproduce
- Potential impact assessment
- Your preferred contact method

**We commit to:**
- Acknowledge receipt within 48 hours
- Investigate and respond within 7 days
- Credit researchers (if desired) after fix
- Provide updates on resolution

---

## 📄 Legal Requests

**For legal inquiries or law enforcement requests:**

**Email:** zarqmessenger@gmail.com
**Subject:** "LEGAL: [Request Type]"

**Please note:**
- We comply with valid legal requests
- Requests must include proper legal documentation
- Due to end-to-end encryption, we cannot access message content
- We can provide: Account metadata, registration info, IP logs (if available)

---

## 💬 Languages

**Currently Supported:**
- English

**Planned:**
- Hindi
- Spanish
- French
- German
- Arabic

*Let us know which languages you'd like to see!*

---

## 📊 Service Status

**Check if there's an ongoing issue:**

Currently, we don't have a status page. If you suspect a service outage:
- Check your internet connection first
- Email us: zarqmessenger@gmail.com
- We'll respond if there's a known issue

**Planned:** Status page at status.zarqmessenger.com

---

## 🙏 Thank You

Thank you for using Zarq Messenger! Your trust in our secure messaging platform means everything to us.

We're committed to:
- 🔒 Protecting your privacy
- 🚀 Continuously improving
- 💬 Listening to your feedback
- 🛡️ Keeping your messages secure

**Your feedback helps us build a better app!**

---

## Quick Contact Summary

| Need Help With | Contact Method | Expected Response |
|---------------|----------------|-------------------|
| Technical Support | zarqmessenger@gmail.com | 7 business days |
| Bug Reports | zarqmessenger@gmail.com | 7 business days |
| Security Issues | zarqmessenger@gmail.com (Subject: SECURITY) | 48 hours |
| Account Issues | zarqmessenger@gmail.com | 7 business days |
| Feature Requests | zarqmessenger@gmail.com | 7 business days |
| Abuse Reports | zarqmessenger@gmail.com (Subject: Report) | 3 business days |
| Legal Requests | zarqmessenger@gmail.com (Subject: LEGAL) | 7 business days |

---

**Zarq Messenger - Zero-access Authenticated Real-time Query**

*Your messages. Zero access. Total privacy.*

**Email:** zarqmessenger@gmail.com

---

**Last Updated:** October 7, 2025
