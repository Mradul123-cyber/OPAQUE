# Terms of Service for OPAQUE

**Last Updated:** September 20, 2026

---

## Welcome to OPAQUE

These Terms of Service ("Terms") govern your use of OPAQUE, a secure messaging application developed by an independent developer in India.

Using OPAQUE requires your agreement to these Terms. If you have questions or concerns, contact us at opaquelabs.in@gmail.com before continuing.

---

## 1. Acceptance of Terms

By creating an account or using OPAQUE, you agree to:
- These Terms of Service
- Our Privacy Policy
- All applicable laws and regulations

**If you are under 18 years old:**
- You must have permission from a parent or legal guardian to use this service
- Your parent/guardian must review and agree to these Terms on your behalf

---

## 2. Eligibility

### Age Requirements

- You must be at least **13 years old** to use OPAQUE
- Users between 13-18 must have parental/guardian consent
- We reserve the right to verify age and request proof if needed

### Account Requirements

- You must provide accurate information during registration
- You must have a valid phone number
- One account per phone number
- You are responsible for maintaining the security of your account

---

## 3. Your Account

### Registration

When you create an account:
- You provide a phone number for verification
- You choose a unique username
- You may optionally add a display name and profile picture

### Account Security

**You are responsible for:**
- Keeping your phone number secure
- Not sharing your account access with others
- Notifying us immediately if you suspect unauthorized access

**We will:**
- Never ask for your password via email or message
- Use industry-standard security measures to protect your account
- Implement end-to-end encryption for your messages

### Account Termination

**You can:**
- Request account deletion at any time by emailing opaquelabs.in@gmail.com
- Stop using the service without penalty

**We can:**
- Suspend or terminate accounts that violate these Terms
- Remove content that violates our policies
- Terminate service for technical, security, or legal reasons

---

## 4. Acceptable Use

### You MAY:

✅ Use OPAQUE for personal communication
✅ Send messages, photos, videos, and files
✅ Make voice and video calls
✅ Create and join group conversations
✅ Customize your profile
✅ Delete your messages and data

### You MAY NOT:

❌ **Illegal Activities:**
- Use the service for any illegal purpose
- Share illegal content (child exploitation, terrorism, etc.)
- Violate any local, national, or international laws

❌ **Harassment and Abuse:**
- Harass, bully, threaten, or intimidate others
- Send spam or unsolicited messages
- Impersonate others or create fake accounts

❌ **Harmful Content:**
- Share viruses, malware, or malicious code
- Share content that promotes violence or hatred
- Share sexually explicit content involving minors

❌ **System Abuse:**
- Attempt to hack, reverse engineer, or compromise the service
- Use automated systems (bots) without permission
- Overload or disrupt servers
- Bypass security measures

❌ **Commercial Misuse:**
- Use the service for unauthorized advertising
- Collect user data without consent
- Resell or redistribute the service

---

## 5. Content and Intellectual Property

### Your Content

**You retain ownership of:**
- Messages you send
- Photos, videos, and files you upload
- Your profile information

**By using OPAQUE, you grant us:**
- License to store and transmit your content (to provide the service)
- Right to delete illegal or violating content
- **Note:** We cannot read your messages due to end-to-end encryption

### Our Intellectual Property

**OPAQUE owns:**
- The app name, logo, and branding
- The software code and design
- All features and functionality

**You may not:**
- Copy, modify, or redistribute the app
- Use our name or logo without permission
- Reverse engineer the application
- Create derivative works

---

## 6. Privacy and Data Protection

### End-to-End Encryption

- All messages are encrypted using the Signal Protocol
- Only you and the recipient can read your messages
- We cannot access your message content

### Data We Collect

Please review our **Privacy Policy** for detailed information about:
- What data we collect
- How we use your data
- How we protect your data
- Your data rights

---

## 7. Service Availability

### No Guarantee

We strive to provide reliable service, but we do not guarantee:
- Uninterrupted access
- Error-free operation
- Availability at all times

### Service Changes

We may:
- Add, modify, or remove features
- Update the app regularly
- Perform maintenance (with notice when possible)

### Service Termination

We reserve the right to:
- Discontinue the service at any time
- Provide notice before termination when possible
- Not be liable for service interruptions

---

## 8. Third-Party Services

OPAQUE uses:
- **Firebase** (Google) for authentication
- **Firebase Cloud Messaging** for push notifications

These services have their own Terms of Service and Privacy Policies:
- Firebase Terms: https://firebase.google.com/terms
- Google Privacy: https://policies.google.com/privacy

---

## 9. Liability and Disclaimers

### Disclaimer of Warranties

**THE SERVICE IS PROVIDED "AS IS" AND "AS AVAILABLE."**

We do not warrant:
- The service will meet your requirements
- The service will be uninterrupted or error-free
- Messages will always be delivered
- Data will never be lost

### Limitation of Liability

**TO THE MAXIMUM EXTENT PERMITTED BY LAW:**

We are not liable for:
- Lost messages or data
- Service interruptions
- Damages resulting from use of the service
- Third-party actions or content
- Loss of profits or business

**Maximum liability is limited to the amount you paid us (currently $0).**

### User Responsibility

**You are responsible for:**
- Your use of the service
- Content you send
- Actions taken on your account
- Backing up important data

---

## 10. Indemnification

You agree to indemnify and hold harmless OPAQUE from:
- Your violations of these Terms
- Your violation of any law or rights of others
- Your use or misuse of the service
- Content you create or share

---

## 11. Dispute Resolution

### Governing Law

These Terms are governed by the laws of India.

### Contact First

Before taking legal action, please contact us at **opaquelabs.in@gmail.com** to resolve the issue.

### Jurisdiction

Any legal disputes will be resolved in courts located in India.

---

## 12. Changes to Terms

### We May Update These Terms

We may modify these Terms at any time. When we do:
- We'll update the "Last Updated" date
- We'll notify you via in-app notification
- Continued use means you accept the changes

### Your Options

If you don't agree with changes:
- Stop using the service
- Delete your account

---

## 13. Termination

### By You

You can stop using OPAQUE at any time by:
- Deleting the app
- Requesting account deletion by emailing opaquelabs.in@gmail.com with your OPAQUE username and the subject "Account deletion"

### By Us

We may terminate or suspend your account if:
- You violate these Terms
- You engage in illegal activities
- Required by law
- For security reasons

**Upon termination:**
- Your account will be disabled
- Your data will be deleted per our retention policy
- You must stop using the service

---

## 14. General Provisions

### Entire Agreement

These Terms, along with our Privacy Policy, constitute the entire agreement between you and OPAQUE.

### Severability

If any provision is found invalid, the remaining provisions remain in effect.

### No Waiver

Our failure to enforce any right doesn't waive that right.

### Assignment

You cannot transfer your rights under these Terms. We may transfer our rights to another entity.

### Force Majeure

We are not liable for failures due to circumstances beyond our control (natural disasters, war, internet outages, etc.).

---

## 15. Contact Information

**For questions, concerns, or legal inquiries:**

**Email:** opaquelabs.in@gmail.com

**Developer:** OPAQUE (Independent Developer, India)

**Response Time:** We aim to respond within 7 business days.

---

## 16. Acknowledgment

By using OPAQUE, you acknowledge that:
- You have read and understood these Terms
- You agree to be bound by these Terms
- You are legally able to enter into this agreement

**Your use of OPAQUE is subject to your agreement to these Terms. For clarification or help closing your account, email opaquelabs.in@gmail.com.**

---

## Summary (Not Legally Binding)

In simple terms:
- 🔒 Be respectful and don't do illegal things
- 🔐 Your messages are private (end-to-end encrypted)
- ⚠️ We're not responsible if something goes wrong
- 📧 Contact us if you have issues
- 🚫 We can ban accounts that violate rules
- 👶 You must be 13+ (with parental permission if under 18)

**The full Terms above are legally binding. This summary is just for convenience.**

---

**OPAQUE**

*Your messages. Zero access. Total privacy.*

---

**Thank you for using OPAQUE! 🔐**
